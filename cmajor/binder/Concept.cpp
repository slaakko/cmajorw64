...
void ConstraintChecker::Visit(ConstructorConstraintNode& constructorConstraintNode)
{
    ...
    lookups.push_back(FunctionScopeLookup(ScopeLookup::this_, firstTypeArgument->BaseType()->ClassInterfaceOrNsScope()));
    ...
    if (!functionCall)
    {
        ...
        if (!functionCall)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ParameterNode* parameterNode = functionConstraintNode.Parameters()[i];
                TypeSymbol* parameterType = ResolveType(parameterNode->TypeExpr(), boundCompileUnit, containerScope);
                lookups.push_back(FunctionScopeLookup(ScopeLookup::this_and_base_and_parent, parameterType->BaseType()->ClassInterfaceOrNsScope()));
                parameterTypes.push_back(parameterType);
                arguments.push_back(std::unique_ptr<BoundExpression>(new BoundTypeExpression(span, parameterType)));
            }
            std::vector<TypeSymbol*> templateArgumentTypes;
...
void ConstraintChecker::Visit(ConvertibleConstraintNode& convertibleConstraintNode)
{
    if (firstTypeArgument && secondTypeArgument)
    {
        FunctionSymbol* conversion = boundCompileUnit.GetConversion(firstTypeArgument, secondTypeArgument, containerScope, span);
...
void ConstraintChecker::Visit(CommonConstraintNode& commonConstraintNode)
{
    if (firstTypeArgument && secondTypeArgument)
    {
        ...
        else
        {
            ...
            {
                FunctionSymbol* conversion = boundCompileUnit.GetConversion(secondTypeArgument, firstTypeArgument, containerScope, span);
                if (conversion && conversion->GetConversionType() == ConversionType::implicit_)
                {
                    commonType->SetType(firstTypeArgument);
                }
