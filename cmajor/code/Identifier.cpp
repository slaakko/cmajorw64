...
#include <cmajor/code/Keyword.hpp>
...
IdentifierGrammar* IdentifierGrammar::Create(cmajor::parsing::ParsingDomain* parsingDomain)
{
...
IdentifierGrammar::IdentifierGrammar(cmajor::parsing::ParsingDomain* parsingDomain_): cmajor::parsing::Grammar(ToUtf32("IdentifierGrammar"), parsingDomain_->GetNamespaceScope(ToUtf32("cmajor.code")), parsingDomain_)
{
    SetOwner(0);
}
...
std::u32string IdentifierGrammar::Parse(const char32_t* start, const char32_t* end, int fileIndex, const std::string& fileName)
{
    ...
    std::u32string result = *static_cast<cmajor::parsing::ValueObject<std::u32string>*>(value.get());
    stack.pop();
    return result;
}

class IdentifierGrammar::IdentifierRule : public cmajor::parsing::Rule
{
public:
...
class IdentifierGrammar::QualifiedIdRule : public cmajor::parsing::Rule
{
public:
...
void IdentifierGrammar::GetReferencedGrammars()
{
    ...
    cmajor::parsing::Grammar* grammar0 = pd->GetGrammar(ToUtf32("cmajor.code.KeywordGrammar"));
    if (!grammar0)
    {
        grammar0 = cmajor::code::KeywordGrammar::Create(pd);
    }
