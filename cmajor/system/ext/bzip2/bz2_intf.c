...
__declspec(dllexport) int32_t bz2_init(int32_t mode, int32_t compressionLevel, int32_t compressionWorkFactor, void** handle)
{
    int32_t ret = BZ_OK;
    if (!handle)
    {
...
__declspec(dllexport) void bz2_done(int32_t mode, void* handle)
{
...
__declspec(dllexport) void bz2_set_input(void* inChunk, uint32_t inAvail, void* handle)
{
    bz_stream* strm = (bz_stream*)handle;
...
__declspec(dllexport) int32_t bz2_compress(void* outChunk, uint32_t outChunkSize, uint32_t* have, uint32_t* outAvail, void* handle, int32_t action)
{
    bz_stream* strm = (bz_stream*)handle;
...
__declspec(dllexport) int32_t bz2_decompress(void* outChunk, uint32_t outChunkSize, uint32_t* have, uint32_t* outAvail, uint32_t* inAvail, void* handle)
{
    bz_stream* strm = (bz_stream*)handle;
    strm->next_out = outChunk;
    strm->avail_out = outChunkSize;
...
__declspec(dllexport) const char* bz2_retval_str(int32_t retVal)
{
    switch (retVal)
    {
